...
def calculate_model_size(model):
    total_params = model.count_params()  # 注: PyTorch中无此函数
    size_bytes = total_params * 4  # 参数数量, 每个参数4字节
...
def FederatedTrain(clients, model_type, outdir, time_window, max_flow_len, dataset_name, epochs='auto', steps='auto',
    ...
    while True:  # training epochs
        ...
        # 服务器发送全局模型给所有客户端的通信量
        round_communication += server_model_size * len(client_subset)
        ...
        for client in client_subset:
            ...
            if client['update'] == True:
                ...
                # start
                # 计算模型差异（客户端 - 服务器）
                local_weights = client['model'].get_weights()
                global_weights = server['model'].get_weights()
                model_diff = [local - global_ for local, global_ in zip(local_weights, global_weights)]
        ...
        # 对客户端压缩后的更新进行解压并聚合
        aggregated_updates = []
        num_layers = len(server['model'].get_weights())
        ...
        current_global_weights = server['model'].get_weights()
        new_global_weights = [global_w + update for global_w, update in zip(current_global_weights, aggregated_updates)]

        # 将更新后的权重设置到服务器模型
        server['model'].set_weights(new_global_weights)
        ...
        print(f"Round {total_rounds}: Communication this round: {round_communication:.4f} MB, Total communication: {total_communication:.4f} MB\n")
